				...
				<div class="wpf-div-tr">
					<div class="wpf-div-td" style="width:60%;">
                        ...
                        <?php if( count($themes) > 1 ): ?>
							<?php if( !$is_active ): ?>
                                <?php if($has_archive): ?>
                                	<a href="<?php echo wp_nonce_url( admin_url( 'admin.php?page=wpforo-themes&wpfaction=wpforo_theme_activate&theme=' . sanitize_text_field($theme_folder) ), 'wpforo-theme-activate' ) ?>" class="wpf-action button"><?php _e('Activate', 'wpforo'); ?></a>
								<?php endif; ?>
                                <a href="<?php echo wp_nonce_url( admin_url( 'admin.php?page=wpforo-themes&wpfaction=wpforo_theme_install&theme=' . sanitize_text_field($theme_folder) ), 'wpforo-theme-install' ) ?>" class="wpf-action button"><?php $has_archive ? _e('Fresh Installation', 'wpforo') : _e('Install', 'wpforo') ?></a>
                                <a href="<?php echo wp_nonce_url( admin_url( 'admin.php?page=wpforo-themes&wpfaction=wpforo_theme_delete&theme=' . sanitize_text_field($theme_folder) ), 'wpforo-theme-delete' ) ?>" class="wpf-delete button" onclick="return confirm('<?php _e('Are you sure you want to delete this theme files?') ?>')"><?php _e('Delete', 'wpforo'); ?></a>
                            <?php else: ?>
                            	<?php if($has_archive): ?>
                            		<a href="<?php echo wp_nonce_url( admin_url( 'admin.php?page=wpforo-themes&wpfaction=wpforo_theme_reset&theme=' . sanitize_text_field($theme_folder) ), 'wpforo-theme-reset' ) ?>" class="wpf-action button" onclick="return confirm('<?php _e('Are you sure you want to reset all settings and style colors to default?') ?>')"><?php _e('Reset Settings', 'wpforo'); ?></a>
								<?php endif; ?>
							<?php endif; ?>
                        <?php endif; ?>
						</div>
